...
// define tooltip2
var tooltip2 = d3.select(".plotarea2").
append("div").
attr("id", "tooltip2").
...
// description
var descript = d3.select(".header2").
append("description");
...
.then(data => {
  ...
  domain(xData).
  range([2 * padding, width2 - 0.5 * padding]);
  var yScale = d3.scaleBand().
  domain(yData).
  range([0, height2 - padding]);
  ...
  .on("mouseover", function (e, d) {
    ...
    tooltip2.html("Year: " + xData[index] +
    ...
    style("top", yScale(yData[index]) + 50 + "px");
  }).
